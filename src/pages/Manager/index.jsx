...
const ManagerPage = ({ onLogout, userData, onProfileUpdated }) => {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [prods, reps, exps, invs, recs, kpis, sts, ords] =
        await Promise.all([
  ...
  const handleCreateOrderHộ = async () => {
    if (!cart.length || !deliveryDate)
      return alert("Vui lòng chọn món và ngày giao!");
    const newOrder = {
  ...
  const filteredInventory = inventory.filter((item) => {
    ...
    if (inventoryAppliedSearch)
      matchText =
        item.sku.toLowerCase().includes(inventoryAppliedSearch.toLowerCase()) ||
        item.name.toLowerCase().includes(inventoryAppliedSearch.toLowerCase());
    let matchLocation =
    ...
    let matchStat =
      filterInventoryStatus === "Cảnh báo tồn kho" ||
      (filterInventoryStatus === "Sắp hết hàng" && item.status === "Sắp hết") ||
      (filterInventoryStatus === "Đã hết hàng" && item.status === "Hết hàng");
  ...
  const filteredRecipes = recipes.filter(
    (r) =>
      (filterRecipeCategory === "Tất cả danh mục" ||
        r.category === filterRecipeCategory) &&
      (recipeAppliedSearch
        ? r.name.toLowerCase().includes(recipeAppliedSearch.toLowerCase())
        : true),
  );
          ...
          {activeManagementTab === "Bảng KPI" && (
            <div className="ck-flex ck-flex-col ck-gap-6">
                ...
                {kpiStats.length > 0 ? (
                  kpiStats.map((stat, idx) => (
                    ...
                    >
                      ...
                      <p
                        className={`ck-text-3xl ck-font-black ck-text-blue-400`}
                      >
                        {stat.value}
                      </p>
                      <span className="ck-text-xs ck-text-gray-500 ck-mt-1">
              ...
              <div className="ck-grid ck-grid-cols-3 ck-gap-6">
                ...
                >
                  <h3 className="ck-text-xl ck-font-bold ck-text-white ck-mb-4">
                    Sản lượng Sản xuất vs Nhu cầu
                  </h3>
                ...
                >
                  <table className="ck-w-full ck-text-left ck-border-collapse">
                    <thead className="ck-bg-gray-800 ck-text-gray-400 ck-text-sm ck-uppercase">
                      <tr>
                        <th className="ck-py-4 ck-px-4">Mã SKU</th>
                        ...
                        .map((prod, idx) => (
                          ...
                          >
                            ...
                            <td className="ck-py-4 ck-px-4 ck-text-blue-400 ck-font-mono">
                              {Number(prod.cogs).toLocaleString()} ₫
                            </td>
                            ...
                            <td className="ck-py-4 ck-px-4 ck-text-center">
                              <button
                                onClick={() => {
                                  ...
                                  setNewMasterProduct(prod);
                                  setShowAddMasterProduct(true);
                                }}
                                className="ck-mr-3 ck-text-gray-400 hover:ck-text-white"
                              >
                                ✏️
                              </button>
                              <button
                                onClick={() =>
                                  handleDeleteMasterProduct(prod.sku)
                                }
                      ...
                      {filteredInventory.length > 0 ? (
                        filteredInventory.map((item, idx) => (
                          <tr
                            ...
                            className={`ck-border-t ck-border-gray-700 ck-cursor-pointer ck-transition-colors hover:ck-bg-gray-800 ${selectedInventoryItem?.sku === item.sku && selectedInventoryItem?.location === item.location ? "ck-bg-gray-800 ck-border-l-4 ck-border-l-yellow-400" : ""}`}
                          >
                            <td className="ck-py-4 ck-px-4 ck-font-mono ck-text-gray-400">
                              {item.sku}
                            </td>
                            ...
                            <td className="ck-py-4 ck-px-4">
                              <span
                                className={`ck-text-xs ck-font-bold ck-px-2 ck-py-1 ck-rounded-md ${item.location === "Bếp Trung Tâm" ? "ck-bg-purple-500-20 ck-text-purple-400" : "ck-bg-blue-500-20 ck-text-blue-400"}`}
                              >
                                {item.location}
                              </span>
                            </td>
                            ...
                            <td
                              className={`ck-py-4 ck-px-4 ck-font-mono ck-font-bold ck-text-right ${item.stock === 0 ? "ck-text-red-500" : item.stock <= item.min ? "ck-text-yellow-400" : "ck-text-white"}`}
                            >
                    ...
                    <div className="ck-space-y-5 ck-text-sm">
                      <div className="ck-bg-gray-800 ck-p-4 ck-rounded-xl ck-border ck-border-gray-700">
                        ...
                        <p className="ck-text-xs ck-text-purple-400 ck-font-bold ck-mb-3">
                          Kho: {selectedInventoryItem.location}
                        </p>
                    ...
                    <tbody className="ck-text-white ck-text-sm">
                      {reports.map((report, idx) => (
                        ...
                        >
                          ...
                          <td className="ck-py-4 ck-px-5 ck-text-center">
                            <button className="ck-btn ck-px-4 ck-py-1 ck-bg-gray-700 hover:ck-bg-gray-600 ck-text-white ck-rounded-lg ck-font-bold ck-text-xs ck-border-none">
                              Tải xuống ⬇
                            </button>
                          </td>
                        </tr>
          ...
          {activeManagementTab === "Quản lý công thức" && (
            <div className="ck-flex ck-flex-col ck-gap-6 ck-h-full ck-animate-fade-in">
              ...
              <div className="ck-flex ck-gap-6 ck-flex-1 ck-items-start ck-transition-all">
                <div
                  className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-overflow-hidden ck-transition-all ck-duration-300"
                  style={{ width: selectedRecipe ? "55%" : "100%" }}
                >
                  <table className="ck-w-full ck-text-left ck-border-collapse">
                    <thead className="ck-bg-gray-800 ck-text-gray-400 ck-text-sm ck-uppercase">
                      <tr>
                    ...
                    <tbody className="ck-text-white ck-text-sm">
                      {filteredRecipes.length > 0 ? (
                        filteredRecipes.map((recipe, idx) => (
                          ...
                          >
                            <td className="ck-py-4 ck-px-4">
                              <span className="ck-block ck-font-bold">
                                {recipe.emoji} {recipe.name}
                              </span>
                              <span className="ck-text-xs ck-font-mono ck-text-gray-400">
                                {recipe.sku}
                              </span>
                            </td>
                            ...
                            <td className="ck-py-4 ck-px-4 ck-text-center">
                              <span
                                className={`ck-px-3 ck-py-1 ck-rounded-full ck-text-xs ck-font-bold ck-border ${recipe.status === "Đã thiết lập" ? "ck-bg-green-500-20 ck-text-green-400 ck-border-green-500-50" : "ck-bg-red-500-20 ck-text-red-400 ck-border-red-500-50"}`}
                              >
                                {recipe.status}
                              </span>
                            </td>

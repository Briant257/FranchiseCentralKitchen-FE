...
const SupplyCoordinatorPage = ({ onLogout, userData, onProfileUpdated }) => {
  ...
  const handleConfirmShipment = async () => {
    if (selectedOrders.length === 0)
      return alert("Vui lòng chọn ít nhất 1 đơn hàng!");
  ...
  return (
    <div className="ck-supply-coordinator-root">
      <div
        className="ck-root ck-min-h-screen ck-bg-black ck-text-white ck-p-6"
              ...
              {[
                ...
                <li key={item.name}>
                  ...
                  >
                    ...
                    {item.count > 0 && (
                      <span className="ck-text-[10px] ck-bg-black/30 ck-px-2 ck-py-0.5 ck-rounded-full">
                        {item.count}
                      </span>
                    )}
                  </button>

...
const CentralKitchenPage = ({ onLogout, userData, onProfileUpdated }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [runsData, catsData, prodsData, ingsData, incsData] =
        await Promise.all([
  ...
  const [filterIncidentPriority, setFilterIncidentPriority] =
    useState("Mức độ ưu tiên");
  ...
  const handleSaveProduct = async () => {
    if (!newProduct.name || !newProduct.price)
      return alert("Vui lòng điền Tên và Giá!");
    try {
      if (editingProduct)
  ...
  const handleSaveIngredient = async () => {
    if (!newIngredient.name || !newIngredient.batch || !newIngredient.exp)
      return alert("Vui lòng điền đủ thông tin lô hàng!");
    try {
      if (editingIngredient)
        await api.updateIngredient(editingIngredient.id, newIngredient);
  ...
  const handleDeleteIngredient = async (ingId) => {
    if (window.confirm("Xóa lô nguyên liệu này?")) {
      try {
  ...
  const handleSaveNewIncident = async () => {
    if (!newIncidentForm.title || !newIncidentForm.reporter)
      return alert("Điền tiêu đề và người báo cáo!");
    try {
  ...
  const filteredIncidents = incidents.filter((i) => {
    ...
    if (incidentAppliedSearch) {
      ...
      matchText =
        i.id.toString().toLowerCase().includes(kw) ||
        i.title.toLowerCase().includes(kw) ||
        i.reporter.toLowerCase().includes(kw);
    }
    return (
      matchText &&
      (filterIncidentPriority === "Mức độ ưu tiên" ||
        i.priority === filterIncidentPriority)
    );
  });
  ...
  return (
    <div className="ck-root ck-min-h-screen ck-bg-black ck-text-white ck-p-6">
      ...
      <div
        className="ck-flex ck-gap-6 ck-w-full ck-relative ck-z-10"
      ...
      >
        ...
        {/* RIGHT CONTENT */}
        <div
          className="ck-flex-1 ck-flex ck-flex-col ck-gap-6 ck-min-w-0"
          style={{ width: "80%" }}
        >
          {/* ======================= TAB TỔNG QUAN ======================= */}
          {activeKitchenTab === "Tổng Quan" && (
            <div className="ck-flex ck-flex-col ck-gap-6 ck-animate-fade-in">
              <div className="ck-grid ck-grid-cols-4 ck-gap-4">
                {[
                  {
                    label: "Tổng suất ăn yêu cầu",
                    value: stats.totalRequested,
                    color: "ck-text-blue-400",
                  },
                  {
                    label: "Mẻ đang nấu",
                    value: stats.cooking,
                  ...
                  <div
                    key={idx}
                    className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-p-5 ck-rounded-2xl ck-text-center ck-flex ck-flex-col ck-items-center ck-justify-center"
                  >
              ...
              <div className="ck-mt-4 ck-pt-6 ck-border-t ck-border-gray-700">
                ...
                <div className="ck-flex ck-gap-6 ck-items-start">
                  <div
                    className={`ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-overflow-hidden ck-transition-all ${showAddCategory || showAddProduct || showAddIngredient ? "ck-w-2/3" : "ck-w-full"}`}
                  >
                    {/* BẢNG DANH MỤC */}
                    {kitchenSubTab === "categories" && (
                      <>
                        ...
                        <table className="ck-w-full ck-text-left ck-border-collapse">
                          ...
                          <tbody className="ck-text-white ck-text-sm">
                            {categories.map((cat, idx) => (
                              <tr
                                key={cat.id}
                                className="ck-border-t ck-border-gray-700 hover:ck-bg-gray-800"
                              >
                                ...
                                <td className="ck-py-4 ck-px-6 ck-text-center">
                                  <div className="ck-flex ck-justify-center ck-gap-3">
                                    <button
                                      ...
                                      className="ck-text-gray-400 hover:ck-text-white"
                                    >
                                      ✏️
                                    </button>
                                    <button
                                      ...
                                      className="ck-text-red-500 hover:ck-text-red-400"
                                    >
                                      🗑️
                                    </button>
                                  </div>
                          ...
                          <tbody className="ck-text-white ck-text-sm">
                            {products.map((p) => (
                              ...
                              >
                                ...
                                <td className="ck-py-4 ck-px-6 ck-font-semibold">
                                  {p.emoji} {p.name}{" "}
                                  <span className="ck-block ck-text-xs ck-text-gray-500">
                                    {p.category}
                                  </span>
                                </td>
                                <td className="ck-py-4 ck-px-6 ck-text-blue-400">
                        ...
                        <table className="ck-w-full ck-text-left ck-border-collapse">
                          <thead className="ck-bg-gray-800 ck-text-gray-400 ck-text-sm ck-uppercase">
                            <tr>
                              ...
                              <th className="ck-py-4 ck-px-6">Lô (Batch)</th>
                              <th className="ck-py-4 ck-px-6">HSD (EXP)</th>
                              <th className="ck-py-4 ck-px-6">Tồn</th>
                          ...
                          <tbody className="ck-text-white ck-text-sm">
                            {ingredients.map((ing) => (
                              ...
                              >
                                ...
                                <td className="ck-py-4 ck-px-6 ck-text-blue-400 ck-font-mono">
                                  {ing.batch}
                                </td>
                                <td className="ck-py-4 ck-px-6 ck-font-mono">
                                  {ing.exp}
                                </td>
                                ...
                                <td className="ck-py-4 ck-px-6">
                                  <span
                                    className={`ck-px-2 ck-py-1 ck-rounded-md ck-text-xs ck-font-bold ck-border ${ing.status === "Còn hạn" ? "ck-bg-green-500-20 ck-text-green-400 ck-border-green-500-50" : ing.status === "Cận date" ? "ck-bg-yellow-500-20 ck-text-yellow-400 ck-border-yellow-500-50" : "ck-bg-red-500-20 ck-text-red-400 ck-border-red-500-50"}`}
                                  >
                                    {ing.status}
                                  </span>
                                </td>
                        ...
                        <div className="ck-space-y-4">
                          <div>
                            ...
                            <input
                              ...
                              className="ck-w-full ck-bg-gray-800 ck-text-white ck-px-4 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 focus:ck-border-orange-500 ck-outline-none"
                              placeholder="VD: Gà quay..."
                            />
                          </div>
                          <button
                            onClick={handleSaveCategory}
                            className="ck-w-full ck-bg-gradient-btn-admin ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none"
                          >
                            Lưu thay đổi
                          </button>
                        </div>
                        ...
                        <div className="ck-space-y-4">
                          <div className="ck-flex ck-gap-3">
                            <div className="ck-w-1/4">
                              ...
                              <input
                                ...
                                className="ck-w-full ck-bg-gray-800 ck-text-white ck-px-3 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 ck-text-center ck-outline-none"
                              />
                            </div>
                            <div className="ck-w-3/4">
                              ...
                              <input
                                ...
                                className="ck-w-full ck-bg-gray-800 ck-text-white ck-px-4 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 focus:ck-border-orange-500 ck-outline-none"
                                placeholder="VD: Gà truyền thống"
                              />
                            </div>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="ck-w-full ck-bg-gray-800 ck-text-blue-400 ck-font-bold ck-px-4 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 focus:ck-border-orange-500 ck-outline-none"
                              placeholder="0"
                            />
                          </div>
                          <div className="ck-flex ck-gap-3">
                          ...
                          <button
                            ...
                            className="ck-w-full ck-mt-2 ck-bg-gradient-btn-admin ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none"
                          >
                            Lưu sản phẩm
                          </button>
                        </div>
                          ...
                          <div className="ck-flex ck-gap-3">
                            <div className="ck-w-1/2">
                              ...
                              <input
                                ...
                                className="ck-w-full ck-bg-gray-800 ck-text-blue-400 ck-font-mono ck-px-4 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 focus:ck-border-orange-500 ck-outline-none"
                                placeholder="B-260..."
                              />
                            </div>
                            <div className="ck-w-1/2">
                ...
                {productionRuns.length === 0 ? (
                  ...
                  productionRuns.map((run) => {
                    const progressPercent =
                      Math.round((run.cookedQty / run.totalQty) * 100) || 0;
                    return (
                      <div
                        key={run.id}
                        className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-p-5 ck-flex ck-flex-col ck-justify-between ck-card-hover"
                      >
                        <div>
                          <div className="ck-flex ck-justify-between ck-items-start ck-mb-4">
                            ...
                            <span
                              className={`ck-px-2 ck-py-1 ck-rounded-md ck-text-[10px] ck-font-bold ck-border ${
                                run.status === "PENDING"
                                  ? "ck-bg-gray-800 ck-text-gray-400 ck-border-gray-600"
                                  : run.status === "COOKING"
                                    ? "ck-bg-orange-500-20 ck-text-orange-400 ck-border-orange-500-50 ck-animate-pulse"
                          ...
                          <div className="ck-flex ck-items-end ck-gap-2 ck-mb-4">
                            <span className="ck-text-4xl ck-font-black ck-text-blue-400">
                              {run.totalQty}
                            </span>
                            <span className="ck-text-sm ck-text-gray-400 ck-pb-1">
                              phần ăn
                            </span>
                          </div>
                          ...
                          <div className="ck-mb-5">
                            <div className="ck-flex ck-justify-between ck-text-[11px] ck-mb-1.5">
                              <span className="ck-text-gray-400 uppercase tracking-wider">
                                Tiến độ nấu
                              </span>
                              <span className="ck-text-white ck-font-bold">
                                {run.cookedQty}/{run.totalQty}
                              </span>
                            </div>
                            <div className="ck-w-full ck-bg-gray-800 ck-rounded-full ck-h-2">
                              <div
                                className={`ck-h-2 ck-rounded-full ck-transition-all ck-duration-700 ${progressPercent === 100 ? "ck-bg-green-500" : "ck-bg-orange-500"}`}
                          ...
                          <div className="ck-bg-gray-800/50 ck-rounded-xl ck-p-3 ck-mb-4">
                            <span className="ck-text-[10px] ck-text-gray-500 ck-mb-2 ck-block ck-font-bold ck-uppercase">
                              Phân bổ chi nhánh:
                            </span>
                            <ul className="ck-space-y-2">
                              {run.details?.map((detail, idx) => (
                                <li
                                  key={idx}
                                  className="ck-flex ck-justify-between ck-items-center ck-text-xs"
                                >
                        ...
                        <div className="ck-mt-2">
                          {run.status === "PENDING" && (
                            <button
                              ...
                              className="ck-w-full ck-bg-gray-800 hover:ck-bg-gray-700 ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border ck-border-gray-600 ck-transition-colors"
                            >
                              🔥 Bắt đầu nấu
                            </button>
                          )}
                          {run.status === "COOKING" && (
                            <button
                              ...
                              className="ck-w-full ck-bg-gradient-btn-admin ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none ck-transition-colors"
                            >
                              ✅ Hoàn thành mẻ
                            </button>
                          )}
                          {run.status === "COMPLETED" && (
                            <button
                              disabled
                              className="ck-w-full ck-bg-gray-800 ck-text-gray-600 ck-py-3 ck-rounded-xl ck-font-bold ck-border-none ck-opacity-50"
                            >
                              Đã xong
                            </button>
                          )}
              ...
              <div className="ck-flex ck-gap-4 ck-items-center">
                <div className="ck-flex ck-flex-1 ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-xl ck-overflow-hidden focus-within:ck-border-red-400 ck-transition-colors">
                  <input
                    ...
                    className="ck-w-full ck-px-4 ck-py-2 ck-outline-none ck-bg-transparent ck-text-white"
                    defaultValue={incidentSearchText}
                    onChange={(e) => setIncidentSearchText(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        setIncidentAppliedSearch(e.target.value);
                    }}
                  />
                  <button
                ...
                <button
                  className="ck-btn ck-px-4 ck-py-2 ck-bg-gradient-btn-admin ck-text-white ck-rounded-xl ck-font-bold ck-flex-shrink-0 ck-border-none"
                  onClick={() => {
              ...
              <div className="ck-flex ck-gap-6 ck-flex-1 ck-items-start ck-transition-all">
                ...
                >
                  <table className="ck-w-full ck-text-center ck-border-collapse">
                    <thead className="ck-bg-gray-800 ck-text-gray-400 ck-text-sm ck-uppercase">
                      <tr>
                        <th className="ck-py-4 ck-px-4 ck-font-semibold">
                          Mã SC
                        </th>
                        <th className="ck-py-4 ck-px-4 ck-font-semibold">
                          Phân loại
                        ...
                        <th className="ck-py-4 ck-px-4 ck-font-semibold ck-text-center">
                          Mức độ
                        </th>
                        <th className="ck-py-4 ck-px-4 ck-font-semibold">
                          Trạng thái
                      ...
                      {filteredIncidents.length > 0 ? (
                        filteredIncidents.map((inc, idx) => (
                          ...
                          >
                            ...
                            <td className="ck-py-4 ck-px-4 ck-font-semibold">
                              {inc.title}
                            </td>
                            <td className="ck-py-4 ck-px-4 ck-text-center">
                              {inc.priority === "Khẩn cấp" ? (
                                <span className="ck-text-red-500 ck-font-black">
                                  🔴 Khẩn cấp
                                </span>
                              ) : inc.priority === "Cao" ? (
                                ...
                                <span className="ck-text-yellow-500 ck-font-semibold">
                                  🟡 Trung bình
                                </span>
                              )}
                            </td>
                            <td className="ck-py-4 ck-px-4">
                              <span
                                className={`ck-px-3 ck-py-1 ck-rounded-full ck-text-xs ck-font-bold ck-border ${inc.status === "Mới" ? "ck-bg-red-500-20 ck-text-red-400 ck-border-red-500-50" : inc.status === "Đang xử lý" ? "ck-bg-orange-500-20 ck-text-orange-400 ck-border-orange-500-50" : "ck-bg-green-500-20 ck-text-green-400 ck-border-green-500-50"}`}
                              >
                                {inc.status}
                              </span>
                            </td>
                  ...
                  >
                    ...
                    <div className="ck-space-y-4">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={newIncidentForm.reporter}
                          onChange={(e) =>
                            setNewIncidentForm({
                              ...newIncidentForm,
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="ck-w-full ck-bg-gray-800 ck-text-white ck-px-4 ck-py-3 ck-rounded-xl ck-border ck-border-gray-700 focus:ck-border-red-500 ck-outline-none ck-min-h-[100px]"
                        ></textarea>
                      </div>
                      <button
                    ...
                    <div className="ck-space-y-4 ck-text-sm">
                      {selectedIncident.priority === "Khẩn cấp" && (
                        <div className="ck-bg-red-500-20 ck-border ck-border-red-500-50 ck-p-3 ck-rounded-lg ck-text-red-400 ck-font-bold ck-flex ck-items-center ck-gap-2">
                          <span className="ck-animate-pulse">⚠️</span> CẦN XỬ LÝ
                          NGAY
                        </div>
                      )}
                      <div>
                        <span className="ck-text-gray-400 ck-block ck-mb-1">
                          Vấn đề:
                        </span>
                        <p className="ck-text-white ck-font-bold ck-text-lg">
                      ...
                      <div className="ck-pt-2">
                        <span className="ck-text-gray-400 ck-block ck-mb-2">
                          Mô tả chi tiết:
                        </span>
                    ...
                    <div className="ck-mt-6 ck-flex ck-flex-col ck-gap-3">
                      {selectedIncident.status === "Mới" && (
                        <button
                          ...
                          className="ck-w-full ck-bg-orange-500 hover:ck-bg-orange-600 ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none ck-transition-colors ck-cursor-pointer"
                        >
                          🔧 Nhận xử lý
                        </button>
                      )}
                      {selectedIncident.status === "Đang xử lý" && (
                        <button
                          ...
                          className="ck-w-full ck-bg-green-500 hover:ck-bg-green-600 ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none ck-transition-colors ck-cursor-pointer"
                        >
                          ✅ Đánh dấu Đã giải quyết
                        </button>
                      )}
                      {selectedIncident.status === "Đã giải quyết" && (
                        <div className="ck-w-full ck-bg-green-500-20 ck-text-green-400 ck-py-3 ck-rounded-xl ck-font-bold ck-text-center">
      ...
      {selectedRecipeRun && (
        <div className="ck-fixed ck-inset-0 ck-bg-black/90 ck-flex ck-items-center ck-justify-center ck-z-50 ck-animate-fade-in">
          <div className="ck-bg-gray-900 ck-border ck-border-blue-500/30 ck-rounded-2xl ck-flex ck-flex-col ck-w-full ck-max-w-md ck-shadow-2xl">
            <div className="ck-p-6 ck-border-b ck-border-gray-800 ck-flex ck-justify-between ck-items-center">
              <div className="ck-flex ck-items-center ck-gap-3">
                ...
                <div>
                  ...
                  <p className="ck-text-xs ck-text-gray-500 uppercase">
              ...
              <button
                ...
                className="ck-text-gray-500 hover:ck-text-white ck-bg-transparent ck-border-none ck-text-xl ck-cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <div
              className="ck-p-6 ck-flex-1 ck-overflow-y-auto ck-scrollbar"
              style={{ maxHeight: "60vh" }}
            >
              <div className="ck-bg-blue-500/10 ck-border ck-border-blue-500/20 ck-p-4 ck-rounded-xl ck-mb-6">
                <p className="ck-text-[11px] ck-text-blue-400 ck-font-bold ck-uppercase ck-mb-1">
                  Tổng sản lượng cần nấu:
                </p>
                <p className="ck-text-2xl ck-font-black ck-text-white">
                  {selectedRecipeRun.totalQty}{" "}
                  <span className="ck-text-sm ck-font-normal ck-text-gray-500">
                    phần ăn
                  </span>
                </p>
              </div>
              ...
              <div className="ck-mb-6">
                <p className="ck-text-[11px] ck-text-gray-500 ck-font-bold ck-uppercase ck-mb-3">
                ...
                <div className="ck-space-y-3">
                  {selectedRecipeRun.bom ? (
                    selectedRecipeRun.bom.map((ing, i) => {
                      const totalNeeded = (
                        ing.qtyPerItem * selectedRecipeRun.totalQty
                      ).toFixed(2);
                      return (
                        <div
                          key={i}
                          className="ck-bg-gray-800/50 ck-border ck-border-gray-800 ck-p-3 ck-rounded-xl ck-flex ck-items-center ck-justify-between"
                        >
                          <span className="ck-text-sm ck-text-gray-300">
                            {ing.name}
                          </span>
                          <div className="ck-text-right">
                            <span className="ck-text-lg ck-font-black ck-text-blue-400 ck-font-mono">
                              {totalNeeded}
                            </span>
                            <span className="ck-text-[10px] ck-text-gray-500 ck-ml-1 uppercase">
                              {ing.unit}
                            </span>
                          </div>
                        </div>
                    ...
                    <p className="ck-text-xs ck-text-gray-500 italic">
                      Dữ liệu công thức đang được cập nhật...
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                <p className="ck-text-[11px] ck-text-gray-500 ck-font-bold ck-uppercase ck-mb-2">
                  Quy trình thực hiện:
                </p>
                <p className="ck-text-xs ck-text-gray-400 ck-leading-relaxed ck-bg-black/30 ck-p-3 ck-rounded-lg ck-border ck-border-gray-800 whitespace-pre-line">
                  {selectedRecipeRun.recipeInstructions ||
                    "Liên hệ quản lý để xem quy trình."}
                </p>
              </div>
            </div>
            ...
            <div className="ck-p-6 ck-bg-gray-800/30 ck-rounded-b-2xl">
              <button
                onClick={() => setSelectedRecipeRun(null)}
                className="ck-w-full ck-bg-blue-600 hover:ck-bg-blue-500 ck-text-white ck-py-3 ck-rounded-xl ck-font-bold ck-border-none ck-transition-colors ck-cursor-pointer"
              >
                Xác nhận & Đóng
              </button>
            </div>

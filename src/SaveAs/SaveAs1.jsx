...
const ManagerPage = ({ onLogout, userData }) => {
  ...
  const handleCreateOrderHộ = async () => {
    ...
    const newOrder = {
      ...
      items: cart.map(i => ({ productId: i.product_id, name: i.name, quantity: i.quantity, price: i.price })),
      status: "pending",
  ...
  const handleDeleteMasterProduct = async (product_id) => { 
    if(window.confirm("Bạn có chắc muốn xóa SP này?")) {
      try { await api.deleteMasterProduct(product_id); loadData(); } catch (error) { alert("Lỗi xóa sản phẩm!"); }
    } 
  };
  ...
  const filteredInventory = inventory.filter(item => {
    let matchText = true;
    if (inventoryAppliedSearch) matchText = item.product_id.toLowerCase().includes(inventoryAppliedSearch.toLowerCase()) || item.name.toLowerCase().includes(inventoryAppliedSearch.toLowerCase());
  ...
  <div className="ck-flex ck-flex-col ck-gap-6 ck-h-full">
    <div className="ck-flex ck-gap-4 ck-items-center">
      <div className="ck-flex ck-flex-1 ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-xl ck-overflow-hidden focus-within:ck-border-red-400 ck-transition-colors">
        <input 
          type="text" 
          placeholder="🔍 Tìm kiếm mã product_id, Tên sản phẩm..." 
          className="ck-w-full ck-px-4 ck-py-2 ck-outline-none" 
          style={{ backgroundColor: '#111827', color: 'white' }} 
          onChange={(e) => setProductAppliedSearch(e.target.value)} 
        />
        <button className="ck-bg-gray-800 hover:ck-bg-gray-700 ck-text-red-400 ck-px-6 ck-font-bold ck-border-l ck-border-gray-700 ck-transition-colors ck-flex-shrink-0">Tìm kiếm</button>
      </div>
      <select 
      ...
      <button 
        ...
        onClick={() => {
          ...
          // FIX: Đổi name -> product_name, cogs -> cost_price, price -> selling_price
          setNewMasterProduct({ product_id: "", product_name: "", category: "Gà rán", cost_price: "", selling_price: "", status: "Đang bán", emoji: "🍔" });
      ...
      <div className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-overflow-hidden ck-transition-all ck-duration-300" style={{ width: showAddMasterProduct ? '66.66%' : '100%' }}>
        <table className="ck-w-full ck-text-left ck-border-collapse">
          <thead className="ck-bg-gray-800 ck-text-gray-400 ck-text-sm ck-uppercase">
            <tr>
              <th className="ck-py-4 ck-px-4">Mã product_id</th>
          ...
          <tbody className="ck-text-white ck-text-sm">
            {masterProducts
              .filter(prod => {
                let matchText = productAppliedSearch ? (prod.product_id?.toLowerCase().includes(productAppliedSearch.toLowerCase()) || prod.product_name?.toLowerCase().includes(productAppliedSearch.toLowerCase())) : true;
                let matchCat = filterProductCategory === "Tất cả danh mục" || prod.category === filterProductCategory;
                return matchText && matchCat;
              })
              .map((prod, idx) => (
              <tr key={prod.product_id || idx} className="ck-border-t ck-border-gray-700 hover:ck-bg-gray-800">
                <td className="ck-py-4 ck-px-4 ck-font-mono ck-text-gray-400">{prod.product_id}</td>
                <td className="ck-py-4 ck-px-4 ck-font-bold">{prod.emoji || '🍔'} {prod.product_name}</td>
                <td className="ck-py-4 ck-px-4">{prod.category || 'Chưa phân loại'}</td>
                <td className="ck-py-4 ck-px-4 ck-text-blue-400 ck-font-mono">{Number(prod.cost_price || 0).toLocaleString()} ₫</td>
                <td className="ck-py-4 ck-px-4 ck-text-green-400 ck-font-mono">{Number(prod.selling_price || 0).toLocaleString()} ₫</td>
                <td className="ck-py-4 ck-px-4 ck-text-center">
                  <button onClick={() => { setEditingMasterProduct(prod); setNewMasterProduct(prod); setShowAddMasterProduct(true); }} className="ck-mr-3 ck-text-gray-400 hover:ck-text-white bg-transparent border-none cursor-pointer">✏️</button>
                  <button onClick={() => handleDeleteMasterProduct(prod.product_id)} className="ck-text-red-500 hover:ck-text-red-400 bg-transparent border-none cursor-pointer">🗑️</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-p-5 ck-animate-fade-in" style={{ width: '33.33%' }}>
          ...
          <div className="ck-space-y-4 ck-text-sm">
            {editingMasterProduct && (
              <div><label className="ck-block ck-text-gray-400 ck-mb-1">Mã product_id</label><input type="text" readOnly value={newMasterProduct.product_id} className="ck-w-full ck-bg-gray-800 ck-text-gray-400 ck-px-3 ck-py-2 ck-rounded-lg ck-border ck-border-gray-700 ck-outline-none" /></div>
            )}
            <div>
              ...
              <div className="ck-flex ck-gap-2">
                ...
                <input type="text" value={newMasterProduct.product_name} onChange={e=>setNewMasterProduct({...newMasterProduct, product_name: e.target.value})} className="ck-flex-1 ck-bg-gray-800 ck-text-white ck-px-3 ck-py-2 ck-rounded-lg ck-border ck-border-gray-700 focus:ck-border-red-500 ck-outline-none" placeholder="Tên SP..." />
              </div>
            </div>
            <div>
                    ...
                    <tbody className="ck-text-white ck-text-sm">
                      {filteredInventory.length > 0 ? (
                        filteredInventory.map((item, idx) => (
                          <tr key={idx} onClick={() => setSelectedInventoryItem(item)} className={`ck-border-t ck-border-gray-700 ck-cursor-pointer ck-transition-colors hover:ck-bg-gray-800 ${selectedInventoryItem?.product_id === item.product_id && selectedInventoryItem?.location === item.location ? 'ck-bg-gray-800 ck-border-l-4 ck-border-l-yellow-400' : ''}`}>
                            <td className="ck-py-4 ck-px-4 ck-font-mono ck-text-gray-400">{item.product_id}</td>
                  ...
                  <div className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-2xl ck-p-5 ck-animate-fade-in" style={{ width: '33.33%' }}>
                    ...
                    <div className="ck-space-y-5 ck-text-sm">
                      <div className="ck-bg-gray-800 ck-p-4 ck-rounded-xl ck-border ck-border-gray-700">
                        <p className="ck-text-xs ck-text-gray-400 ck-font-mono ck-mb-1">{selectedInventoryItem.product_id}</p>
            ...
            <div className="ck-flex ck-flex-col ck-gap-6 ck-h-full ck-animate-fade-in">
              <div className="ck-flex ck-gap-4 ck-items-center">
                <div className="ck-flex ck-flex-1 ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-xl ck-overflow-hidden focus-within:ck-border-orange-400 ck-transition-colors">
                  <input type="text" placeholder="🔍 Tìm mã product_id, Tên sản phẩm..." className="ck-w-full ck-px-4 ck-py-2 ck-outline-none" style={{ backgroundColor: '#111827', color: 'white' }} defaultValue={recipeSearchText} onChange={(e) => setRecipeSearchText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') setRecipeAppliedSearch(e.target.value); }} />
                    ...
                    <tbody className="ck-text-white ck-text-sm">
                      {filteredRecipes.length > 0 ? (
                        filteredRecipes.map((recipe, idx) => (
                          <tr key={idx} onClick={() => setSelectedRecipe(recipe)} className={`ck-border-t ck-border-gray-700 ck-cursor-pointer ck-transition-colors hover:ck-bg-gray-800 ${selectedRecipe?.product_id === recipe.product_id ? 'ck-bg-gray-800 ck-border-l-4 ck-border-l-orange-500' : ''}`}>
                            <td className="ck-py-4 ck-px-4"><span className="ck-block ck-font-bold">{recipe.emoji} {recipe.name}</span><span className="ck-text-xs ck-font-mono ck-text-gray-400">{recipe.product_id}</span></td>
        ...
        {stores.map(store => (
          <div key={store.store_id} 
            ...
            <div className="ck-flex ck-justify-between ck-mb-4">
              <div className="ck-p-3 ck-bg-gray-800 ck-rounded-xl group-hover:ck-bg-red-500/10 transition-colors">
            ...
            <div className="ck-flex ck-justify-between ck-items-center ck-pt-4 ck-border-t ck-border-gray-800">
              {/* FIX: id -> store_id */}
              <span className="ck-text-xs ck-font-mono ck-text-gray-600">{store.store_id}</span>
        ...
        <div className="ck-flex ck-justify-between ck-items-center ck-bg-gray-900 ck-p-4 ck-rounded-2xl ck-border ck-border-gray-700">
          <div className="ck-flex ck-items-center ck-gap-4">
            ...
            <div>
              ...
              {/* FIX: id -> store_id */}
              <p className="ck-text-xs ck-text-gray-500 ck-font-mono">{selectedStore.store_id} | {selectedStore.address}</p>
            </div>
          </div>
          {!isOrderingForStore && (
        ...
        {isOrderingForStore ? (
          <div className="ck-grid ck-grid-cols-3 ck-gap-6 ck-animate-slide-up">
            <div className="ck-col-span-2 ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-3xl ck-p-8">
              <div className="ck-flex ck-justify-between ck-mb-8 items-center">
                ...
                <div className="ck-relative">
                  ...
                  <input 
                    ...
                    className="ck-w-64 ck-bg-gray-800 ck-text-white ck-pl-12 ck-pr-4 ck-py-2 ck-rounded-xl ck-border ck-border-gray-700 outline-none" 
              ...
              <div className="ck-grid ck-grid-cols-2 ck-gap-4 ck-max-h-[500px] ck-overflow-y-auto ck-scrollbar ck-pr-2">
                {masterProducts
                  .filter(p => p.product_name?.toLowerCase().includes(searchTermHộ.toLowerCase()))
                  .map(p => (
                  <div key={p.product_id} className="ck-bg-gray-800 ck-p-5 ck-rounded-2xl ck-flex ck-justify-between ck-items-center hover:ck-bg-gray-700 ck-transition-colors ck-border ck-border-transparent hover:ck-border-red-500/50">
                    <div className="ck-flex ck-gap-4">
                      ...
                      <div>
                        {/* FIX: p.name -> p.product_name */}
                        <p className="ck-font-bold ck-text-white text-sm">{p.product_name}</p>
                        {/* FIX: p.price -> p.selling_price */}
                        <p className="ck-text-xs ck-text-blue-400 ck-font-mono mt-1">{Number(p.selling_price).toLocaleString()}đ</p>
                      </div>
                    </div>
            ...
            <div className="ck-bg-gray-900 ck-p-8 ck-rounded-3xl ck-border ck-border-red-500/20 shadow-2xl ck-flex ck-flex-col">
              <h3 className="ck-text-xl ck-font-black ck-mb-6 ck-flex ck-items-center ck-gap-3 text-white">
                <ShoppingCart className="ck-text-red-500" size={24}/> Giỏ hàng hộ
              </h3>
              <div className="ck-flex-1 ck-space-y-4 ck-mb-6 ck-max-h-64 ck-overflow-y-auto ck-scrollbar pr-2">
                {cart.length === 0 ? (
                  ...
                  <div key={item.product_id} className="ck-flex ck-justify-between ck-items-center ck-bg-gray-800 ck-p-3 ck-rounded-xl">
                    <div className="ck-flex-1">
                      ...
                      {/* FIX: item.price -> item.selling_price */}
                      <p className="ck-text-[10px] ck-text-gray-500">{item.quantity} x {Number(item.selling_price).toLocaleString()}đ</p>
                    </div>
                    <span className="ck-text-red-400 ck-font-black ck-font-mono">{(item.selling_price * item.quantity).toLocaleString()}đ</span>
                  </div>
                ))}
              </div>
              <div className="ck-border-t ck-border-gray-800 ck-pt-6 ck-space-y-4">
                <input type="date" className="ck-w-full ck-bg-gray-800 ck-text-white ck-p-3 ck-rounded-xl border-none outline-none" onChange={(e) => setDeliveryDate(e.target.value)} />
                <div className="ck-flex ck-justify-between ck-items-center ck-mb-4">
                   ...
                   <span className="ck-text-2xl ck-font-black ck-text-orange-400">
                     {cart.reduce((s, i) => s + i.selling_price * i.quantity, 0).toLocaleString()}đ
                   </span>
                </div>
          ...
          <div className="ck-bg-gray-900 ck-border ck-border-gray-700 ck-rounded-3xl ck-overflow-hidden shadow-2xl">
            <div className="ck-p-6 ck-bg-gray-800/50 ck-border-b ck-border-gray-700 ck-flex ck-justify-between items-center">
               ...
               <button className="ck-text-blue-400 ck-font-bold text-xs bg-transparent border-none ck-cursor-pointer hover:underline">Xuất báo cáo Store</button>
            </div>
            <table className="ck-w-full ck-text-left ck-border-collapse">
              ...
              <tbody className="ck-text-sm text-white">
                {allOrders
                  /* FIX: storeId -> store_id, id -> store_id */
                  .filter(o => o.store_id === selectedStore.store_id)
                  .map(order => (
                  <tr key={order.order_id} className="ck-border-t ck-border-gray-800 hover:ck-bg-gray-800/50 ck-transition-colors">
                    <td className="ck-p-5 ck-font-mono ck-text-blue-400 ck-font-bold">{order.order_id}</td>
                    <td className="ck-p-5 ck-text-gray-400">{order.created_at || order.date}</td>
                    <td className="ck-p-5 ck-text-white ck-font-medium">{order.delivery_date || order.deliveryDate}</td>
                    <td className="ck-p-5 ck-text-right ck-font-black ck-text-orange-400">
                      {Number(order.total_amount || order.total).toLocaleString()}đ
                    </td>
